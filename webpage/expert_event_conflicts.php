...
$event_query = "SELECT vid.animal_id, vid.watermarked_filename AS video_name, obs.video_id, obs.event_id, e.name AS event_name, obs.start_time_s AS start_sec, obs.end_time_s AS end_sec FROM timed_observations AS obs JOIN observation_types AS e ON e.id = event_id JOIN video_2 AS vid ON vid.id = video_id WHERE obs.expert = 1 AND obs.start_time_s >= 0 AND obs.end_time_s >= obs.start_time_s";
...
while ($event_row = $event_result->fetch_assoc()) {
    ...
    if (!empty($conflicts)) {
        $match_query = "SELECT * FROM timed_observations WHERE expert = 1 AND video_id = $video_id AND event_id IN ($conflicts) AND (start_time_s BETWEEN $start_sec AND $end_sec OR end_time_s BETWEEN $start_sec AND $end_sec)";
