...
{
	$age = urldecode($_POST['regis1']); //Age
	$sex = urldecode($_POST['regis2']); //Sex
	$education = urldecode($_POST['regis3']); //Education
	$profession = urldecode($_POST['regis4']); //Profession
	if($profession == "Student (Please describe field of study below)" || $profession == "Retired (Please describe past employment below)" || $profession == "Other (Please describe below)")
	{
		...
		{
			$profession .= "; " . urldecode($_POST['regis4tcon']);
		}
		else
		{
	...
	$english = urldecode($_POST['regis5']); //Is english your first language?
	$eng_fluent = urldecode($_POST['regis6']); //If not, are you fluent?

	$population = urldecode($_POST['regis7']); //Population of current city
	...
	$participation = urldecode($_POST['regis10']); //How would you participate?
...
{
	$participation = urldecode($_POST['gold0']);
	...
	{
		$speciesdata .= urldecode($_POST['gold1tcon']);
	}
	...
	{
		$speciesdata .= urldecode($_POST['gold2tcon']);
	}
	...
	{
		$speciesdata .= urldecode($_POST['gold3tcon']);
	}
	$speciesdata .= "]";
	
	$perweek = urldecode($_POST['gold4']);
	$persitting = urldecode($_POST['gold5']);
	$vidspeed = urldecode($_POST['gold6']);
	
	$browser = urldecode($_POST['gold7']);
	if($browser == "Other")
	{
		$browser .= " [Browser Listed: " . urldecode($_POST['gold7tcon']) . "]";
	}
	
	$otheract = urldecode($_POST['gold8']);
	if($otheract == "Yes")
	{
		$otheract .= "; " . urldecode($_POST['gold8tcon']);
	}
	
	$interesting = urldecode($_POST['gold9']);
	if($interesting == "Yes")
	{
		$temp = "gold-10-";
		$options = array("I enjoy contributing to scientific knowledge"=>0, "I enjoy the computer science component"=>0, "I like earning credit and badges"=>0, "I enjoy the wildlife/biology component"=>0, "Other (Please Explain)"=>1);
	...
	$learnednew = urldecode($_POST['gold11tcon']);
	if(empty($learnednew))
	{
		$learnednew = "[No response]";
	}
	
	$userexperience = urldecode($_POST['gold12tcon']);
	...
	$otherspecies = urldecode($_POST['gold14tcon']);
	if(empty($otherspecies))
	{
		$otherspecies = "[No response]";
	}
	
	$recommendation = urldecode($_POST['gold15']);
	...
	{
		$recommendation .= "; " . urldecode($_POST['gold15tcon']);
	}
...
{
	$video_con = mysql_connect("wildlife.und.edu", "wildlife_user", "gr0u\$e\$", TRUE);
